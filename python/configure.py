...
def copy_cpp_file():
  ...
  cpp_dst_path = root_path + "/python/merlin_kv/tensorflow/core/lib/merlin-kv/"
  _cmd = "mkdir -p " + cpp_dst_path

  ret = os.system(_cmd)
  if ret != 0:
    raise ValueError(
        'Error happened when creating cpp path:{}.'.format(cpp_dst_path))
  _cmd = "cp -f " + root_path + "/BUILD " + cpp_dst_path
  ret = os.system(_cmd)
  if ret != 0:
    raise ValueError('Error happened when copy cpp fils.')
  _cmd = "cp -rf " + root_path + "/cpp " + cpp_dst_path
  ret = os.system(_cmd)
  if ret != 0:
    raise ValueError('Error happened when copy cpp fils.')
...
def create_build_configuration():
  print()
  print("Configuring NVIDIA Merlin-KV to be built from source...")

  # Merlin-KV always needs GPU.
  ...
  print()
  print("Build configurations successfully written to", _MKV_BAZELRC, ":\n")
  print(pathlib.Path(_MKV_BAZELRC).read_text())
