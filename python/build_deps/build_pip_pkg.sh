...
if is_windows; then
  PIP_FILE_PREFIX="bazel-bin/build_pip_pkg.exe.runfiles/merlin_kv/"
else
  PIP_FILE_PREFIX="bazel-bin/build_pip_pkg.runfiles/merlin_kv/"
fi
...
function main() {
  ...
  if is_windows; then
    from=$(cygpath -w ${PIP_FILE_PREFIX}merlin_kv)
    to=$(cygpath -w "${TMPDIR}"/merlin_kv)
    start robocopy //S "${from}" "${to}" //xf *_test.py
    sleep 5
  else
    rsync -avm -L --exclude='*_test.py' ${PIP_FILE_PREFIX}merlin_kv "${TMPDIR}"
  fi
