...
#
"""NVIDIA Merlin Hierarchical Key-Value Storage.

A hierarchical key-value storage library designed for the large models in recommenders systems.
Merlin-KV stores the key-value (embedding) on the HBM and Host memory (support SSD/NVMe coming soon).
"""
...
def get_project_name_version():
  ...
  with open(os.path.join(base_dir, "merlin_kv", "version.py")) as fp:
...
setup(
    ...
    author_email="merlinteam@nvidia.com",
    ...
    license="Apache 2.0",
    keywords="NVIDIA hashtable Key-Value GPU recommenders",
)
