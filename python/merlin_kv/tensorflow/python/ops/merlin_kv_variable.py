...
class Variable(base.Trackable):
  ...
  def __init__(
    ...
    self.kv_creator = kv_creator if kv_creator else mkv.MerlinKVCreator()
    ...
    if "GPU" in self.devices[0].upper():
      ...
      raise TypeError('Merlin-KV need to run on NVIDIA GPUs.')
    ...
    with ops.name_scope(scope_name, "MerlinKV_Variable"):
      with ops.colocate_with(None, ignore_existing=True):
        for idx in range(len(self.devices)):
          with ops.device(self.devices[idx]):
            mht = None
            if not issubclass(self.kv_creator.__class__, mkv.KVCreator):
  ...
  def get_trainable_by_name(self, name):
    ...
    params = mkv.get_variable('foo', dim=4, initializer=init)
    optimizer = tf.keras.optimizers.Adam(1E-3)
    optimizer = mkv.DynamicEmbeddingOptimizer(optimizer)

    @tf.function
    def loss_fn(ids):
      emb = mkv.embedding_lookup(params, ids, name='user_embedding')
...
@tf_export("tensorflow.get_variable")
def get_variable(
    name,  # unique
    key_dtype=dtypes.int64,
    value_dtype=dtypes.float32,
