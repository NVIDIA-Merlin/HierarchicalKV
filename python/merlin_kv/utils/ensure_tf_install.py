...
from merlin_kv.version import MIN_TF_VERSION
from merlin_kv.version import MAX_TF_VERSION
...
def _check_tf_version():
  """Warn the user if the version of TensorFlow used is not supported.
    ...
    we support this TensorFlow version if the user uses only Python code.
    """

  if "dev" in tf.__version__:
    warnings.warn(
        "You are currently using a nightly version of TensorFlow ({}). \n"
        "NVIDIA Merlin-KV offers no support for the nightly versions of "
  ...
  warnings.warn(
      "NVIDIA Merlin-KV supports using Python ops for all Tensorflow versions "
      ...
      "either change the TensorFlow version or the Merlin-KV's version. \n"
      "You can find the compatibility matrix in Merlin-KV's readme:\n"
      "https://github.com/NVIDIA-Merlin/merlin-kv".format(
