...
def get_required_tf_version():
  try:
    pkg = pkg_resources.get_distribution("merin-kv")
  except pkg_resources.DistributionNotFound:
    try:
      pkg = pkg_resources.get_distribution("merin-kv-gpu")
    except pkg_resources.DistributionNotFound:
      # Force return for 'Test with bazel' on CI.
      warnings.warn(
          "Fail to get Merlin-KV package information, if you are running on "
          "bazel test mode, please ignore this warning, \nor you should check "
          "Merlin-KV installation.",
          UserWarning,
      )
  ...
  for x in pkg_info:
    if x.name in ["tensorflow", "tensorflow-gpu"]:
      return x.specs[0][1]
  assert False, "Fail to get required TensorFlow version of Merlin-KV!"
...
def get_devices(device_type="GPU"):
  if hasattr(tf.config, "list_physical_devices"):
    ...
    warnings.warn(
        "You are currently using TensorFlow {} which Merlin-KV cann't get the devices correctly.\n"
        "So we strongly recommend that you use the version supported by the Merlin-KV statement "
        "To do that, refer to the readme: "
        "https://github.com/NVIDIA-Merlin/merlin-kv"
...
def get_path_to_datafile(path):
  """Get the path to the specified file in the data dependencies.
    ...
    The path is relative to merlin_kv/

    Args:
      path: a string resource path relative to merlin_kv/
    Returns:
...
class LazySO:
  ...
  def display_warning_if_incompatible(self):
    ...
    warnings.warn(
        ...
        "\n"
        "Merlin-KV has compiled its custom ops against TensorFlow {}, "
        ...
        "\n\n"
        "The last solution is to compile the NVIDIA Merlin-KV "
